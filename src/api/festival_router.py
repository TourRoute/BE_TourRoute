...
async def get_festival_info():
    ...
    get_query = my_col.find({}, {"_id": 0})
    if get_query:
        return [x for x in get_query]
    else:
...
async def delete_bookmark(fastival_name: str, token: str = Header(default=None)):
    user_email = check_token(token)
    my_db = my_client["touroute"]
    my_col = my_db["bookmark"]
    bookmark = my_col.find_one({"_id": 0, "user_email": user_email, "f_name": fastival_name})
    if bookmark:
        my_col.delete_one({"user_email": user_email, "f_name": fastival_name})
